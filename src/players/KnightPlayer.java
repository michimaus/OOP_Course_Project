...
public class KnightPlayer extends StandardPlayer {
    ...
    public KnightPlayer(final String type, final int posR, final int posC, final int playerId) {
        super(type, posR, posC, playerId);
        maxHp = Constants.KNIGHT_HP;
        currentHp = maxHp;
        strategy = KnightStrategy.getInstance();
    }

    @Override
    public final void calculateStrike(final PlayerVisitor heroSpells, final StandardPlayer opponent,
                                final char land) {
        opponent.getExecuted(heroSpells, this);
        opponent.getSlamed(heroSpells, this);
    }
}
